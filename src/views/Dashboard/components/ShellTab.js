...
const ShellTab = ({ shellIx }) => {
  ...
  let etherscan = config.network == 43114 ? 'https://snowtrace.io/address/' + config.shells[shellIx].shell
  ...
  return ( <>
    ...
    <StyledShellTab>
      <Overview>
        <OverviewSection>
          <LabelledValue label="Toroid Reserves" value={ liqTotal} />
        </OverviewSection>
        <OverviewSection>
      ...
      <StyledRows>
        <Row head>
          ...
          <span style={{ flex: 1, textAlign: 'right' }}> Toroid Reserves </span>
